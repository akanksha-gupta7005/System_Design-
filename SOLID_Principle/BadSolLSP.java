...
// This is break the Liskov Substitution Principle. This class norrowing the
// behavior of parent class instead of expend it.
...
class BankClient {
    ...
    void processTransaction() {
        for (Account acc : accounts) {
            ...
            // Try to solve liskov problem using bad way
            // Open Close Principle break in future, whenever we try to change or add new functionality in code 
            if (acc instanceof FixedDepositeAccount) {
                System.out.println("Skipping this account!");
            } else {
                try {
                    acc.withdraw(3000);
                } catch (Exception e) {
